...
export async function encodeRequest<T>(body: T) {
    ...
    return {
        data: new Uint8Array([0, ...encodeUInt32(expires), ...nonce, ...keypair.publicKey, ...data]),
        nonce,
...
export async function decodeRequest<T>(data: Uint8Array, schema: z.ZodSchema<T>) {
    ...
    const expires = decodeUInt32(data.slice(1, 1 + 4));
    const nonce = data.slice(1 + 4, 1 + 4 + nacl.box.nonceLength);
    const publicKey = data.slice(1 + 4 + nacl.box.nonceLength, 1 + 4 + nacl.box.nonceLength + nacl.box.publicKeyLength);
    const rawBody = data.slice(1 + 4 + nacl.box.nonceLength + nacl.box.publicKeyLength);
    if (expires < Math.floor(Date.now() / 1000)) {
        console.log(expires, Math.floor(Date.now() / 1000));
        return null;
    }
    let body: any
    try {
...
export async function encryptResponse<T>(body: T, nonce: Uint8Array, publicKey: Uint8Array) {
    ...
    const expires = Math.floor(Date.now() / 1000) + 5 * 60; // 5 minutes
    return new Uint8Array([1, ...encodeUInt32(expires), ...keypair.publicKey, ...encrypted]);
}
...
export async function decryptResponse<T>(data: Uint8Array, nonce: Uint8Array, secretKey: Uint8Array, schema: z.ZodSchema<T>) {
    ...
    const expires = decodeUInt32(data.slice(1, 1 + 4));
    const senderPublicKey = data.slice(1 + 4, 1 + 4 + nacl.box.publicKeyLength);
    const encrypted = data.slice(1 + 4 + nacl.box.publicKeyLength);
    if (expires < Math.floor(Date.now() / 1000)) {
        return null;
    }
