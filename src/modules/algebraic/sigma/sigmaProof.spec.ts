...
describe('Sigma Proof', () => {
  ...
  describe('createSigmaProof', () => {
    it('should create a valid proof for single statement', () => {
      ...
      expect(proof.protocol).toEqual(protocol);
      expect(proof.context.nonce).toEqual(nonce);
      expect(proof.context.usage).toBe('test_proof');
    ...
    it('should create a valid proof for multiple statements', () => {
      ...
      expect(proof.protocol).toEqual(protocol);
      expect(proof.context.usage).toBe('test_multiple_statements');
    ...
    it('should create a valid proof for complex protocol', () => {
      ...
      expect(proof.context.usage).toBe('complex_proof');
    });
  });
  ...
  describe('verifySigmaProof', () => {
    it('should verify a valid proof for single statement', () => {
      ...
      const result = verifySigmaProof(proof, publicVariables);
    ...
    it('should reject proof with wrong challenge', () => {
      ...
      const publicVariables = { P, G, H };
      const result = verifySigmaProof(tamperedProof, publicVariables);
    ...
    it('should reject proof with different nonce', () => {
      ...
      // Change the nonce in context
      const tamperedProof = {
        ...proof,
        context: {
          ...proof.context,
          nonce: encodeUTF8('different_nonce')
        }
      };
  ...
  describe('createPublicSigmaProof', () => {
    it('should create proof and extract public variables', () => {
      ...
      expect(proof.protocol).toEqual(protocol);
      expect(proof.context.usage).toBe('public_proof');
  ...
  describe('Edge Cases', () => {
    ...
    it('should produce different proofs with different nonces', () => {
      ...
      const publicVariables = { P, G, H };
      expect(verifySigmaProof(proof1, publicVariables).isValid).toBe(true);
      expect(verifySigmaProof(proof2, publicVariables).isValid).toBe(true);
    });
    ...
    it('should produce different proofs with different usage strings', () => {
      ...
      expect(proof1.challenge).not.toEqual(proof2.challenge);
      expect(proof1.context.usage).toBe('usage1');
      expect(proof2.context.usage).toBe('usage2');
    ...
    it('should normalize only statements, not usage or variable names', () => {
      ...
      expect(verifySigmaProof(proof1, publicVariables).isValid).toBe(true);
      expect(verifySigmaProof(proof2, publicVariables).isValid).toBe(true);
      expect(verifySigmaProof(proof3, publicVariables).isValid).toBe(true);
    });
    ...
    it('should demonstrate statement normalization protection', () => {
      ...
      const publicVariables = { P, G, H };
      expect(verifySigmaProof(proof, publicVariables).isValid).toBe(true);
